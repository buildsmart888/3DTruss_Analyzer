...
public sealed class ShellViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    public ShellViewModel() { Settings = _settingsStore.Load(_settingsPath); _stage = "Physical"; RecentProjects = new(Settings.RecentProjectPaths.Where(File.Exists)); _documents.Changed += (_, _) => Refresh(); _history.Changed += (_, _) => { OnChanged(nameof(CanUndo)); OnChanged(nameof(CanRedo)); }; _autosave.Start(TimeSpan.FromMinutes(Math.Clamp(Settings.AutosaveMinutes, 1, 60)), () => { try { _documents.Autosave(); } catch { } }); }
